// js/config/data.js - Centralized data configuration

const CONFIG = {
  // Site metadata
  site: {
    title: "Abdulmalik Apata | Backend Software Engineer",
    description:
      "Backend Software Engineer specializing in distributed systems and high-performance computing",
    author: "Abdulmalik Apata",
    email: "apataomotayo0@gmail.com",
  },

  // Navigation items
  navigation: [
    {
      icon: "home",
      label: "Home",
      href: "/",
      svg: '<path d="M15 21v-8a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v8"></path><path d="M3 10a2 2 0 0 1 .709-1.528l7-5.999a2 2 0 0 1 2.582 0l7 5.999A2 2 0 0 1 21 10v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path>',
    },
    { type: "divider" },
    {
      icon: "twitter",
      label: "Twitter",
      href: "https://x.com/@belmont_browne",
      target: "_blank",
      svg: '<path d="M12.6.75h2.454l-5.36 6.142L16 15.25h-4.937l-3.867-5.07-4.425 5.07H.316l5.733-6.57L0 .75h5.063l3.495 4.633L12.601.75Zm-.86 13.028h1.36L4.323 2.145H2.865z"></path>',
    },
    {
      icon: "github",
      label: "Github",
      href: "https://github.com/Brownie44l1",
      target: "_blank",
      svg: '<path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"></path><path d="M9 18c-4.51 2-5-2-7-2"></path>',
    },
    {
      icon: "resume",
      label: "Resume",
      href: "/Apata_Abdulmalik-CV.pdf",
      target: "_blank",
      svg: '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7Z"></path><path d="M14 2v4a2 2 0 0 0 2 2h4"></path><path d="M10 9H8"></path><path d="M16 13H8"></path><path d="M16 17H8"></path>',
    },
    { type: "divider" },
    {
      type: "cta",
      label: "My Blog",
      mobileLabel: "Blog",
      href: "https://belmont-blog.vercel.app",
      target: "_blank",
    },
  ],

  // Projects data
  projects: [
    {
      id: "http-server",
      slug: "http-server",
      title: "High-Performance HTTP Server",
      subtitle: "60k requests/sec HTTP/1.1 server in Go",
      description:
        "Minimal HTTP/1.1 server achieving 60k req/sec through careful optimization of Go's runtime, eliminating lock contention, and implementing zero-copy techniques",
      longDescription:
        "A from-scratch HTTP/1.1 server implementation in Go that achieves approximately 60,000 requests per second on modest hardware (Intel Pentium N4200). The project explores low-level performance optimization including per-connection buffer reuse, zero-copy header parsing, and understanding Go's netpoller internals. Through iterative benchmarking and profiling, achieved a 216% performance improvement by eliminating sync.Pool contention and implementing connection-local buffers. The journey involved failed experiments with C (epoll) and Rust to ultimately understand that performance comes from working with the language's runtime, not against it.",
      image: "assets/images/projects/http-server.png",
      images: [
        "assets/images/projects/http-server.png",
        "assets/images/projects/http-server-bench.png",
      ],
      technologies: [
        "Go",
        "HTTP/1.1",
        "Network Programming",
        "Performance Optimization",
        "Benchmarking",
      ],
      date: "2026",
      type: "Systems Programming",
      client: "Personal Research",
      url: null,
      github: "https://github.com/Brownie44l1/http-server",
      blogUrl: "https://belmont-blog.vercel.app/projects/http-server.html",
      featured: true,
      metrics: [
        { label: "Throughput", value: "60k req/sec" },
        { label: "Performance Gain", value: "+216%" },
        { label: "Latency (avg)", value: "1.76ms" },
        { label: "Concurrent Connections", value: "100" },
      ],
    },
    {
      id: "facial-emotion-recognition",
      slug: "facial-emotion-recognition",
      title: "Facial Emotion Recognition System",
      subtitle: "Deep learning model for pediatric emotion detection",
      description:
        "Academic research project developing a facial emotion recognition model for Nigerian children aged 6-12, using ResNet50 with CBAM attention mechanism",
      longDescription:
        "An ongoing research project building a facial emotion recognition system specifically trained on Nigerian children between ages 6-12. The system uses ResNet50 architecture enhanced with Convolutional Block Attention Module (CBAM) for improved feature extraction. The model is exported to ONNX format for production deployment and exposed via FastAPI for real-time inference. Data collection is being conducted in partnership with elementary schools (data remains private for ethical reasons). The system includes both web and mobile interfaces for accessibility. This project addresses the underrepresentation of African children in emotion recognition datasets and explores transfer learning challenges across different demographic groups.",
      image: "assets/images/projects/fer-ui.png",
      images: [
        "assets/images/projects/fer-ui.png",
        "assets/images/projects/fer-demo.png",
      ],
      technologies: [
        "Python",
        "PyTorch",
        "ResNet50",
        "CBAM",
        "ONNX",
        "FastAPI",
        "Deep Learning",
        "Computer Vision",
      ],
      date: "Nov 2025 - Present",
      type: "ML Research",
      client: "Academic Project",
      url: "https://fer-emotionai.vercel.app",
      github: "https://github.com/Brownie44l1/fer-frontend",
      blogUrl: null,
      featured: true,
      metrics: [
        { label: "Model", value: "ResNet50 + CBAM" },
        { label: "Target Age", value: "6-12 years" },
        { label: "Status", value: "In Development" },
        { label: "Deployment", value: "ONNX + FastAPI" },
      ],
    },
    {
      id: "voicebox",
      slug: "voicebox",
      title: "VoiceBox - Student Complaint Management",
      subtitle:
        "Anonymous complaint tracking system for educational institutions",
      description:
        "Full-stack complaint management platform enabling students to submit anonymous or identified complaints with admin resolution tracking",
      longDescription:
        "A comprehensive complaint management system designed for educational institutions. Students can submit complaints either anonymously or with their identity, addressing various campus issues. The system provides a tracking code for anonymous submissions, allowing students to monitor resolution status without revealing their identity. Features include an admin dashboard for complaint review, categorization, and resolution tracking. Originally built with PHP and MySQL, the project was rebuilt using modern JavaScript stack (Express.js + MongoDB) for easier deployment and maintenance. The system promotes transparency and accountability in educational settings while protecting student privacy.",
      image: "assets/images/projects/voicebox-ui.png",
      images: [
        "assets/images/projects/voicebox-ui.png",
        "assets/images/projects/voicebox-admin.png",
      ],
      technologies: [
        "Node.js",
        "Express.js",
        "MongoDB",
        "JavaScript",
        "HTML/CSS",
        "Authentication",
      ],
      date: "2026",
      type: "Full-Stack Web App",
      client: "Educational Institutions",
      url: "https://voicebox.vercel.app",
      github: "https://github.com/Brownie44l1/voicebox-backend",
      blogUrl: null,
      featured: true,
      metrics: [
        { label: "Submission Types", value: "Anonymous + Identified" },
        { label: "Tech Stack", value: "MERN" },
        { label: "Features", value: "Tracking Codes" },
      ],
    },
    {
      id: "inventory-tracker",
      slug: "inventory-tracker",
      title: "SMSE Inventory Management",
      subtitle: "Inventory tracking system for small/medium enterprises",
      description:
        "Full-stack inventory management system with stock tracking, order management, and reporting capabilities",
      longDescription:
        "A comprehensive inventory tracking solution built for small and medium-scale enterprises (SMSE). The system provides real-time stock level monitoring, order processing, product categorization, and automated reporting. Features include low-stock alerts, purchase order management, supplier tracking, and sales analytics. Built with PHP and MySQL backend with TailwindCSS for a modern, responsive interface. The system helps businesses maintain optimal inventory levels, reduce stockouts, and make data-driven purchasing decisions.",
      image: "assets/images/projects/inventory-ui.png",
      images: ["assets/images/projects/inventory-ui.png"],
      technologies: ["PHP", "MySQL", "HTML", "TailwindCSS", "JavaScript"],
      date: "2025",
      type: "Full-Stack Web App",
      client: "SME Sector",
      url: null, // Will be updated after deployment
      github:
        "https://github.com/Brownie44l1/Inventory-Management-System-for-SMSE",
      blogUrl: null,
      featured: true,
      metrics: [
        { label: "Target Users", value: "SMEs" },
        { label: "Features", value: "Stock + Orders + Reports" },
        { label: "Status", value: "Pending Deployment" },
      ],
    },
    {
      id: "social-media-baba",
      slug: "social-media-baba",
      title: "TheSocialMediaBaba Portfolio",
      subtitle: "Professional social media management portfolio website",
      description:
        "Modern, conversion-focused portfolio website for a social media expert showcasing services, case studies, and client testimonials",
      longDescription:
        "A sleek, professional portfolio website built for TheSocialMediaBaba, a social media management expert. The site effectively showcases data-driven content strategies, client success stories, and service offerings across all major social platforms. Features include an integrated Calendly booking system for client consultations, responsive design for mobile and desktop viewing, interactive testimonials section, and detailed case studies highlighting real metrics and results. Built as a static site with modern HTML, CSS, and JavaScript, optimized for fast loading and SEO. The clean, conversion-optimized design emphasizes trust-building through client logos, testimonials, and proven results across 50+ brands.",
      image: "assets/images/projects/socialmedia-baba-ui.png",
      images: [
        "assets/images/projects/socialmedia-baba-ui.png",
        "assets/images/projects/socialmedia-baba-services.png",
        "assets/images/projects/socialmedia-baba-testimonials.png",
      ],
      technologies: ["HTML", "CSS", "JavaScript", "TailwindCSS", "Vercel"],
      date: "2026",
      type: "Static Portfolio Website",
      client: "Social Media Professional",
      url: "https://thesocialmediababa.vercel.app/",
      github: null,
      blogUrl: null,
      featured: false,
      metrics: [
        { label: "Client Reach", value: "50+ Brands" },
        { label: "Type", value: "Frontend Only" },
        { label: "Status", value: "Live" },
      ],
    },
    {
      id: "slicecheck",
      slug: "slicecheck",
      title: "SliceCheck",
      subtitle: "PDF & DOCX plagiarism splitter tool",
      description:
        "A free web tool that splits PDF and DOCX files into word-limited chunks ready for pasting into plagiarism checkers like Duplichecker, SmallSEOTools, and PaperRater.",
      longDescription:
        "SliceCheck is a client-side utility that solves a common pain point for students and writers — free plagiarism checkers impose strict word limits, making it tedious to check long documents. The app accepts PDF or DOCX uploads (up to 50 MB), extracts the text, and intelligently splits it into paste-ready chunks based on configurable word counts and sentence-boundary tolerance. Users can customise chunk sizes to match popular checkers (Duplichecker at 1000 words, PaperRater at 5000, etc.), strip repeating headers and footers, and download all chunks as .txt files. The tool processes pages progressively in the background so users can start working immediately. Built as a static site with no server-side processing — all extraction and splitting happens in the browser.",
      image: "assets/images/projects/slicecheck-ui.png",
      images: [
        "assets/images/projects/slicecheck-ui.png",
        "assets/images/projects/slicecheck-ui-1.png",
        "assets/images/projects/slicecheck-ui-2.png",
      ],
      technologies: ["HTML", "CSS", "JavaScript", "Vercel"],
      date: "2026",
      type: "Static Web Tool",
      client: "Personal / Open Use",
      url: "https://slicecheck.vercel.app",
      github: "https://github.com/Brownie44l1/slicecheck-frontend",
      blogUrl: null,
      featured: false,
      metrics: [
        { label: "Max File Size", value: "50 MB" },
        { label: "Type", value: "Frontend Only" },
        { label: "Status", value: "Live" },
      ],
    },
  ],

  // Skills
  skills: [
    "Go",
    "Python",
    "Node.js",
    "JavaScript",
    "PHP",
    "PostgreSQL",
    "MongoDB",
    "MySQL",
    "Redis",
    "Docker",
    "FastAPI",
    "Express.js",
    "HTTP/Network Programming",
    "Deep Learning",
    "PyTorch",
    "System Design",
    "Performance Optimization",
    "REST APIs",
  ],

  // Tech icons for marquee
  techIcons: [
    `<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 448 512" class="w-7 h-7"><path d="M439.8 200.5c-7.7-30.9-22.3-54.2-53.4-54.2h-40.1v47.4c0 36.8-31.2 67.8-66.8 67.8H172.7c-29.2 0-53.4 25-53.4 54.3v101.8c0 29 25.2 46 53.4 54.3 33.8 9.9 66.3 11.7 106.8 0 26.9-7.8 53.4-23.5 53.4-54.3v-40.7H226.2v-13.6h160.2c31.1 0 42.6-21.7 53.4-54.2 11.2-33.5 10.7-65.7 0-108.6zM286.2 404c11.1 0 20.1 9.1 20.1 20.3 0 11.3-9 20.4-20.1 20.4-11 0-20.1-9.2-20.1-20.4.1-11.3 9.1-20.3 20.1-20.3zM167.8 248.1h106.8c29.7 0 53.4-24.5 53.4-54.3V91.9c0-29-24.4-50.7-53.4-55.6-35.8-5.9-74.7-5.6-106.8.1-45.2 8-53.4 24.7-53.4 55.6v40.7h106.9v13.6h-147c-31.1 0-58.3 18.7-66.8 54.2-9.8 40.7-10.2 66.1 0 108.6 7.6 31.6 25.7 54.2 56.8 54.2H101v-48.8c0-35.3 30.5-66.4 66.8-66.4zm-6.7-142.6c-11.1 0-20.1-9.1-20.1-20.3.1-11.3 9-20.4 20.1-20.4 11 0 20.1 9.2 20.1 20.4s-9 20.3-20.1 20.3z"></path></svg>`,
    `<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 448 512" class="w-7 h-7"><path d="M0 32v448h448V32H0zm243.8 349.4c0 43.6-25.6 63.5-62.9 63.5-33.7 0-53.2-17.4-63.2-38.5l34.3-20.7c6.6 11.7 12.6 21.6 27.1 21.6 13.8 0 22.6-5.4 22.6-26.5V237.7h42.1v143.7zm99.6 63.5c-39.1 0-64.4-18.6-76.7-43l34.3-19.8c9 14.7 20.8 25.6 41.5 25.6 17.4 0 28.6-8.7 28.6-20.8 0-14.4-11.4-19.5-30.7-28l-10.5-4.5c-30.4-12.9-50.5-29.2-50.5-63.5 0-31.6 24.1-55.6 61.6-55.6 26.8 0 46 9.3 59.8 33.7L368 290c-7.2-12.9-15-18-27.1-18-12.3 0-20.1 7.8-20.1 18 0 12.6 7.8 17.7 25.9 25.6l10.5 4.5c35.8 15.3 55.9 31 55.9 66.2 0 37.8-29.8 58.6-69.7 58.6z"></path></svg>`,
    `<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 512 512" class="w-7 h-7"><path d="M418.2 177.2c-5.4-1.8-10.8-3.5-16.2-5.1.9-3.7 1.7-7.4 2.5-11.1 12.3-59.6 4.2-107.5-23.1-123.3-26.3-15.1-69.2.6-112.6 38.4-4.3 3.7-8.5 7.6-12.5 11.5-2.7-2.6-5.5-5.2-8.3-7.7-45.5-40.4-91.1-57.4-118.4-41.5-26.2 15.2-34 60.3-23 116.7 1.1 5.6 2.3 11.1 3.7 16.7-6.4 1.8-12.7 3.8-18.6 5.9C38.3 196.2 0 225.4 0 255.6c0 31.2 40.8 62.5 96.3 81.5 4.5 1.5 9 3 13.6 4.3-1.5 6-2.8 11.9-4 18-10.5 55.5-2.3 99.5 23.9 114.6 27 15.6 72.4-.4 116.6-39.1 3.5-3.1 7-6.3 10.5-9.7 4.4 4.3 9 8.4 13.6 12.4 42.8 36.8 85.1 51.7 111.2 36.6 27-15.6 35.8-62.9 24.4-120.5-.9-4.4-1.9-8.9-3-13.5 3.2-.9 6.3-1.9 9.4-2.9 57.7-19.1 99.5-50 99.5-81.7 0-30.3-39.4-59.7-93.8-78.4zM282.9 92.3c37.2-32.4 71.9-45.1 87.7-36 16.9 9.7 23.4 48.9 12.8 100.4-.7 3.4-1.4 6.7-2.3 10-22.2-5-44.7-8.6-67.3-10.6-13-18.6-27.2-36.4-42.6-53.1 3.9-3.7 7.7-7.2 11.7-10.7zM167.2 307.5c5.1 8.7 10.3 17.4 15.8 25.9-15.6-1.7-31.1-4.2-46.4-7.5 4.4-14.4 9.9-29.3 16.3-44.5 4.6 8.8 9.3 17.5 14.3 26.1zm-30.3-120.3c14.4-3.2 29.7-5.8 45.6-7.8-5.3 8.3-10.5 16.8-15.4 25.4-4.9 8.5-9.7 17.2-14.2 26-6.3-14.9-11.6-29.5-16-43.6zm27.4 68.9c6.6-13.8 13.8-27.3 21.4-40.6s15.8-26.2 24.4-38.9c15-1.1 30.3-1.7 45.9-1.7s31 .6 45.9 1.7c8.5 12.6 16.6 25.5 24.3 38.7s14.9 26.7 21.7 40.4c-6.7 13.8-13.9 27.4-21.6 40.8-7.6 13.3-15.7 26.2-24.2 39-14.9 1.1-30.4 1.6-46.1 1.6s-30.9-.5-45.6-1.4c-8.7-12.7-16.9-25.7-24.6-39s-14.8-26.8-21.5-40.6zm180.6 51.2c5.1-8.8 9.9-17.7 14.6-26.7 6.4 14.5 12 29.2 16.9 44.3-15.5 3.5-31.2 6.2-47 8 5.4-8.4 10.5-17 15.5-25.6zm14.4-76.5c-4.7-8.8-9.5-17.6-14.5-26.2-4.9-8.5-10-16.9-15.3-25.2 16.1 2 31.5 4.7 45.9 8-4.6 14.8-10 29.2-16.1 43.4zM256.2 118.3c10.5 11.4 20.4 23.4 29.6 35.8-19.8-.9-39.7-.9-59.5 0 9.8-12.9 19.9-24.9 29.9-35.8zM140.2 57c16.8-9.8 54.1 4.2 93.4 39 2.5 2.2 5 4.6 7.6 7-15.5 16.7-29.8 34.5-42.9 53.1-22.6 2-45 5.5-67.2 10.4-1.3-5.1-2.4-10.3-3.5-15.5-9.4-48.4-3.2-84.9 12.6-94zm-24.5 263.6c-4.2-1.2-8.3-2.5-12.4-3.9-21.3-6.7-45.5-17.3-63-31.2-10.1-7-16.9-17.8-18.8-29.9 0-18.3 31.6-41.7 77.2-57.6 5.7-2 11.5-3.8 17.3-5.5 6.8 21.7 15 43 24.5 63.6-9.6 20.9-17.9 42.5-24.8 64.5zm116.6 98c-16.5 15.1-35.6 27.1-56.4 35.3-11.1 5.3-23.9 5.8-35.3 1.3-15.9-9.2-22.5-44.5-13.5-92 1.1-5.6 2.3-11.2 3.7-16.7 22.4 4.8 45 8.1 67.9 9.8 13.2 18.7 27.7 36.6 43.2 53.4-3.2 3.1-6.4 6.1-9.6 8.9zm24.5-24.3c-10.2-11-20.4-23.2-30.3-36.3 9.6.4 19.5.6 29.5.6 10.3 0 20.4-.2 30.4-.7-9.2 12.7-19.1 24.8-29.6 36.4zm130.7 30c-.9 12.2-6.9 23.6-16.5 31.3-15.9 9.2-49.8-2.8-86.4-34.2-4.2-3.6-8.4-7.5-12.7-11.5 15.3-16.9 29.4-34.8 42.2-53.6 22.9-1.9 45.7-5.4 68.2-10.5 1 4.1 1.9 8.2 2.7 12.2 4.9 21.6 5.7 44.1 2.5 66.3zm18.2-107.5c-2.8.9-5.6 1.8-8.5 2.6-7-21.8-15.6-43.1-25.5-63.8 9.6-20.4 17.7-41.4 24.5-62.9 5.2 1.5 10.2 3.1 15 4.7 46.6 16 79.3 39.8 79.3 58 0 19.6-34.9 44.9-84.8 61.4zm-149.7-15c25.3 0 45.8-20.5 45.8-45.8s-20.5-45.8-45.8-45.8c-25.3 0-45.8 20.5-45.8 45.8s20.5 45.8 45.8 45.8z"></path></svg>`,
    `<svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 640 512" class="w-7 h-7"><path d="M349.9 236.3h-66.1v-59.4h66.1v59.4zm0-204.3h-66.1v60.7h66.1V32zm78.2 144.8H362v59.4h66.1v-59.4zm-156.3-72.1h-66.1v60.1h66.1v-60.1zm78.1 0h-66.1v60.1h66.1v-60.1zm276.8 100c-14.4-9.7-47.6-13.2-73.1-8.4-3.3-24-16.7-44.9-41.1-63.7l-14-9.3-9.3 14c-18.4 27.8-23.4 73.6-3.7 103.8-8.7 4.7-25.8 11.1-48.4 10.7H2.4c-8.7 50.8 5.8 116.8 44 162.1 37.1 43.9 92.7 66.2 165.4 66.2 157.4 0 273.9-72.5 328.4-204.2 21.4.4 67.6.1 91.3-45.2 1.5-2.5 6.6-13.2 8.5-17.1l-13.3-8.9zm-511.1-27.9h-66v59.4h66.1v-59.4zm78.1 0h-66.1v59.4h66.1v-59.4zm78.1 0h-66.1v59.4h66.1v-59.4zm-78.1-72.1h-66.1v60.1h66.1v-60.1z"></path></svg>`,
  ],

  // Social links
  socialLinks: [
    {
      name: "Twitter",
      url: "https://x.com/@belmont_browne",
      icon: '<path d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z"></path>',
    },
    {
      name: "LinkedIn",
      url: "https://www.linkedin.com/in/apata-abdulmalik-715351259/",
      icon: '<path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-13h4v2a4 4 0 0 1 3-2z"></path><rect width="4" height="13" x="2" y="8"></rect><circle cx="4" cy="4" r="2"></circle>',
    },
    {
      name: "GitHub",
      url: "https://github.com/Brownie44l1",
      icon: '<path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"></path><path d="M9 18c-4.51 2-5-2-7-2"></path>',
    },
    {
      name: "Email",
      url: "mailto:apataomotayo0@gmail.com",
      icon: '<rect width="20" height="16" x="2" y="4" rx="2"></rect><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"></path>',
    },
  ],

  // Work experience
  workExperience: [
    {
      period: "Dec 2024 - Present",
      role: "Freelancing on the",
      company: "Internet",
    },
    {
      period: "Dec 2023 - March 2024",
      role: "Frontend Intern at",
      company: "Deazy Tech Solutions",
    },
  ],
};

// Export for use in other modules
if (typeof module !== "undefined" && module.exports) {
  module.exports = CONFIG;
}
